...
import { downloadZipUrl, exportToTelegram, getDiscordBotInviteUrl, uploadToDiscord } from "@/lib/api";
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 pb-8">
      ...
      <div className="space-y-3 rounded-2xl bg-white/80 p-4 shadow-card">
        ...
        <div className="space-y-2">
          ...
          {dcStep === "invited" && (
            <div className="space-y-2 rounded-xl border border-[#5865F2]/30 bg-[#5865F2]/5 p-3">
              <p className="text-xs font-semibold text-[#5865F2]">
                ① 봇 초대창에서 서버 선택 후 "허가" 클릭
              </p>
              <p className="text-xs text-kakao-brown/60">
                ② Discord 설정 → 고급 → 개발자 모드 ON → 서버 우클릭 → ID 복사
              </p>
              <input
                type="text"
                value={guildId}
                onChange={(e) => setGuildId(e.target.value)}
                placeholder="서버 ID 입력 (예: 123456789012345678)"
                className="w-full rounded-lg border border-kakao-brown/20 px-3 py-2 text-sm outline-none focus:border-[#5865F2]"
              />
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => { setDcStep("idle"); setGuildId(""); }}
                  className="flex-1 rounded-lg border border-kakao-brown/15 py-2 text-xs"
                >
                  취소
                </button>
                <button
